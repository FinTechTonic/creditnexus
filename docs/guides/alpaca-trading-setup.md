...
If Alpaca keys are not set, **trading** uses `MockTradingAPIService`. If `ALPACA_DATA_ENABLED` is false or Alpaca returns no data, **market data** falls back to **yahooquery** (see [Stock Prediction Setup](stock-prediction-setup.md)).
...
See [Stock Prediction Setup](stock-prediction-setup.md) for backtest and prediction config.
