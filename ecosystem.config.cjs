...
module.exports = {
  apps: [
    ...
    // MCP Server (x402 payment-protected tools). Run from repo root: pm2 start ecosystem.config.cjs --only mcp-server
    {
      name: 'mcp-server',
      script: useVenv ? venvPython : (isWindows ? 'cmd' : 'python'),
      args: useVenv ? ['demo_mcp/server/server.py'] : (isWindows ? ['/c', 'python', 'demo_mcp/server/server.py'] : ['demo_mcp/server/server.py']),
      cwd: projectRoot,
      interpreter: 'none',
      env: {
        NODE_ENV: 'development',
