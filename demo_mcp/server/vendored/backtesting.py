"""
Backtesting for trading strategies (vendored from CreditNexus).
Uses vendored market_data (yahooquery) and trading_strategies. No DB cache.
"""
...
try:
    from server.vendored.trading_strategies import STRATEGIES, get_trading_signals
except ImportError:
    from demo_mcp.server.vendored.trading_strategies import STRATEGIES, get_trading_signals
...
def run_backtest_from_data_source(
    ...
    """
    Load OHLCV via vendored get_historical_data, then run_backtest. No cache.
    """
    try:
    ...
    return run_backtest(df, strategy=strategy, initial_capital=initial_capital)
